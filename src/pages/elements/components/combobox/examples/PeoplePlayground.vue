		...
		>
			<template #item="{ item }">
				<div class="flex min-w-0 items-center gap-3">
					<img :src="item.avatar" alt="" class="size-8 rounded-full bg-skin-surface object-cover" />
					<span class="min-w-0">
						<span class="block truncate font-medium">{{ item.label }}</span>
						<span class="block truncate text-xs text-skin-muted">{{ item.role }}</span>
					</span>
				</div>
			</template>
		</ElCombobox>
		<p class="text-xs text-skin-muted">Selected value: <code class="text-skin-primary">{{ data.modelValue || '—' }}</code></p>
	</div>
</template>
