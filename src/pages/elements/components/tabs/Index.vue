	...
	<ElementsLayout>
		<DocPage name="Tabs" tagline="Roving-tabindex tab list with managed ARIA, keyboard navigation, and panel linking." tag="<element-tabs>">
			<DocSection eyebrow="Playground" title="Try every prop live">
				...
				>
					<template #overview><p class="text-sm text-muted-foreground">Overview panel content.</p></template>
					<template #pricing><p class="text-sm text-muted-foreground">Pricing panel content.</p></template>
					<template #changelog><p class="text-sm text-muted-foreground">Changelog panel content.</p></template>
				</Playground>
			</DocSection>
			...
			<DocSection eyebrow="Demo" title="Live preview">
				<Demo>
					<div class="w-full max-w-md">
						<ElTabs v-model="active" :tabs="tabs">
							<template #overview>
								<div class="rounded-xl border border-border bg-background p-5 text-sm text-muted-foreground">Overview content. Tabs preserve focus and announce panel changes to assistive tech.</div>
							</template>
							<template #pricing>
								<div class="rounded-xl border border-border bg-background p-5 text-sm text-muted-foreground">Pricing content. Use the arrow keys to move between tabs.</div>
							</template>
							<template #changelog>
								<div class="rounded-xl border border-border bg-background p-5 text-sm text-muted-foreground">Changelog content. Each panel is keyboard-reachable.</div>
							</template>
						</ElTabs>
					</div>
				</Demo>
