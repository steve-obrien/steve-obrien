			...
			<table class="w-full text-left text-sm">
				...
				<tbody class="divide-y divide-border">
					<tr v-for="a in attrs" :key="a.name">
						...
						<td class="px-4 py-3 font-mono text-[12.5px] text-secondary">{{ a.type || '—' }}</td>
						<td class="px-4 py-3 text-secondary">{{ a.description || '—' }}</td>
					</tr>
				</tbody>
			</table>
		</div>
		<p v-if="name" class="text-[11px] text-muted">
