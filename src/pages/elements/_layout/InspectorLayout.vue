			...
			<div class="flex h-12 items-center justify-between gap-4 px-4">
				<div class="flex min-w-0 items-center gap-4">
					<RouterLink to="/elements" class="flex shrink-0 items-center gap-2 text-foreground">
						<span class="grid size-6 place-items-center rounded-md bg-primary-skin text-primary text-[10px] font-bold tracking-tight">E</span>
				...
				<nav class="flex shrink-0 items-center gap-1 text-sm">
					...
					:class="(l.exact ? route.path === l.to : route.path.startsWith(l.match || l.to))
						...
						: 'text-secondary hover:text-foreground'">{{ l.label }}</RouterLink>
					<BtnLightDarkMode />
					<RouterLink
					to="/elements/pricing"
					class="hidden rounded-full bg-primary-skin px-2.5 py-1 text-xs font-medium text-primary hover:opacity-90 md:inline">Get access</RouterLink>
				</nav>
			</div>
		</header>
