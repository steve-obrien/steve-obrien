			...
			<table class="min-w-full text-left text-sm">
				...
				<tbody class="divide-y divide-border">
					<tr v-for="r in rows" :key="r.name">
						<td class="px-4 py-3 font-mono text-[12.5px] text-foreground">{{ r.name }}</td>
						<td class="px-4 py-3 font-mono text-[12.5px] text-muted-foreground">{{ r.type }}</td>
						<td class="px-4 py-3 font-mono text-[12.5px] text-muted-foreground">{{ r.default || '—' }}</td>
