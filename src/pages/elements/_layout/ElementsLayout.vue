...
<template>
	<div class="min-h-screen bg-background text-foreground">
		<header class="sticky top-0 z-30 border-b border-border/60 bg-background/80 backdrop-blur">
			<div class="mx-auto flex h-16 w-full items-center justify-between px-6">
				<RouterLink to="/elements" class="flex items-center gap-2 text-foreground">
					<span class="grid size-7 place-items-center rounded-lg bg-primary-skin text-primary text-xs font-bold tracking-tight">E</span>
					<span class="font-semibold tracking-tight">elements</span>
					<span class="ml-2 hidden rounded-full bg-secondary-skin px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-secondary ring-1 ring-border md:inline">v1.0</span>
				</RouterLink>
				<!-- Mobile menu-->
				...
				<Teleport to="body">
					...
					<RouterLink
						to="/elements/pricing"
						class="ml-2 hidden sm:inline not-first:rounded-full bg-primary-skin px-3 py-1.5 text-sm font-medium text-primary hover:opacity-90 "
					>Get access</RouterLink>
					<RouterLink
						to="/"
						class="ml-1 hidden lg:inline text-xs text-muted hover:text-foreground "
		...
		<footer class="border-t border-border/60">
			<div class="mx-auto flex w-full max-w-7xl flex-col items-start justify-between gap-4 px-6 py-10 text-sm text-secondary sm:flex-row sm:items-center">
				<div class="flex items-center gap-3">
					<span class="grid size-6 place-items-center rounded-md bg-primary-skin text-primary text-[10px] font-bold">E</span>
