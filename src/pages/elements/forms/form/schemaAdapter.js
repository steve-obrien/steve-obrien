...
function plainValidator(schema) {
	...
	return {
		name: 'schema',
		validate(value) {
			if ((plain.optional || plain.required === false) && (value === '' || value == null)) return true;
...
function fieldOptions(options, path, key) {
	return options.fields?.[path] || options.fields?.[key] || {};
}
...
function componentFor(schema, path, options) {
	...
	const override = fieldOptions(options, path, path.split('.').pop()).component || plain.component;
...
function fieldPropsFor(schema, path, options) {
	...
	const plain = raw(schema) || {};
	const field = fieldOptions(options, path, key);
